...
void computeIntersections()
{
	...
	/*
	if (hmd) {
			offS = projectionMatrix[2][0] / projectionMatrix[2][1];
		...
		*/
	//offS = -0;
	//vec3 cameraPosition = preMult(vec3(gl_FragCoord.x - offS, gl_FragCoord.y, 1.0), viewToWorldMatrix);
	//vec3 cameraPosition = preMult(vec3(0, 0, 0.0), gl_ModelViewMatrixInverse);
	//vec3 pos = vec3(cameraPosition.x, cameraPosition.y, abs(cameraPosition.z));
	//vec3 cam_pos = convertPositionToTextureCoords(pos);
	//if (all(greaterThanEqual(cam_pos, vec3(0.0, 0.0, 0.0))) && all(lessThanEqual(cam_pos, vec3(50.0, 50.0, 50.0)))) {
	//	insideVolume = true;
	//} else {
	//	insideVolume = false;
	...
	//if (all(greaterThanEqual(cameraPosition, cuboidPosition)) && all(lessThanEqual(cameraPosition, cuboidPosition + cuboidDimensions)))
	//{
	//insideVolume = false;
	if (insideVolume) {
		...
		//vec3 insideCamPos = preMult(vec3(gl_FragCoord.x, gl_FragCoord
		//endPt = cameraPosition;
		startPt = campos.xyz;
		//endPt = preMult(vec3(gl_FragCoord.x , gl_FragCoord.y, 1.0), viewToWorldMatrix);
		//startPt = cameraPosition;
		
		//vec3 camerap = (viewToWorldMatrix[3][0],viewToWorldMatrix[3][1], viewToWorldMatrix[3][2]);
		//startPt = camerap;
		...
		//startPt = preMult(vec3(gl_FragCoord.x, gl_FragCoord.y, 12.0), viewToWorldMatrix);
        
		//startPt = vertex.xyz;
		endPt = vertex.xyz;
		
		//float dist1 = distance(endPt, startPt); 
		//vec3 ray1 = normalize(endPt - startPt);
		
		//endPt = startPt + (ray1 * (dist1 * 2));
		//endPt = vertex.xyz + vec3(0.0, 00.0, 0.0);
		//startPt = endPt + (ray1 * dist);
		
		//startPt = preMult(vec3(gl_FragCoord.x - (100 * offS), gl_FragCoord.y, 0.0), viewToWorldMatrix);
		//Ray ray;
		//ray.origin = startPt;
		
		//vec3 zFarPt = preMult(vec3(gl_FragCoord.x - offS, gl_FragCoord.y, 1.0), viewToWorldMatrix);
       ...
       // ray.sign = vec3(int(ray.inverseDir.x < 0.0), int(ray.inverseDir.y < 0.0), int(ray.inverseDir.z < 0.0));
		...
		//endPt = preMult(vertex.xyz, viewToWorldMatrix);
		//endPt = vertex.xyz;
	...
	// Compute the back intersection point
		...
		//ray.origin = preMult(vec3(gl_FragCoord.x , gl_FragCoord.y, 0.0), viewToWorldMatrix);
		//ray.origin = vec3(vertex.xyz) * vec3(1.0,1.0,1.0);
		//ray.origin = preMult(vec3(gl_FragCoord.x , gl_FragCoord.y, 0.0), gl_ModelViewMatrixInverse);
	...
	totalIterations = clamp(totalIterations, 1.0, 256.0);
	//totalIterations = 25;
	//totalIterations = 256.0;
	if (!insideVolume) {
		step = (endPt - startPt) / float(totalIterations - 1.0);
	} else {
		totalIterations = clamp(totalIterations, 1.0, 128.0);
		//totalIterations /= 4;
		step = (endPt - startPt) / float(totalIterations - 1.0);
		//step = (startPt - endPt) / float(((totalIterations - 1.0) / 0.05));
		//step /= 1.5;
		//step = (endPt - startPt) / float((totalIterations) - 1.0);
		//step *= -0.01;
	}
...
void renderComposite()
{
	...
	float raylength = length(endPt - pos) / 100;
	float distTraveled = 0;
	float stepLen = length(step);
	//vec3 minCornerBB = vec3(0.0, 0.0, 0.0);
	vec3 minCornerBB = cuboidPosition;
	vec3 maxCornerBB = cuboidPosition + cuboidDimensions;
	//if (pos.x >= 0.0) {
		//discard;
	//}
    ...
    while (totalIterations > 0.0)
    {
        while (totalIterations > 0.0)
        {
			...
			//vec3 toOrig = (endPt - pos);
			//vec3 newOrig = endPt - (pos + step);
			distTraveled += stepLen;
			
			/*
			if (distance(startPt, pos) > distance(startPt, (pos+step))) {
				totalIterations = 0.0;
			}
			*/
			
			if (distTraveled > raylength) {
				//totalIterations = 0.0;
			}
